...
type PaymentUpdater interface {
	OutboxUpdatePaymentTx(ctx context.Context, idemKey string, payload any) error
}
...
func (c *ConfirmService) ValidateWebhook(ctx context.Context, rawData []byte) error {
	...
	// Outbox pattern
	if err = c.paymentupdr.OutboxUpdatePaymentTx(ctx, data.IdempotencyKey, data); err != nil {
